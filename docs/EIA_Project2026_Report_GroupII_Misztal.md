...
The `compute_total_energy` subroutine was optimized using OpenMP shared-memory parallelization to accelerate the computationally expensive $\mathcal{O}(N^2)$ Lennard-Jones (LJ) non-bonded interactions and the backbone torsional energy evaluations. To ensure thread safety and prevent data race conditions during the global energy summation, local loop variables (such as the inner index `j`, squared distance `r2`, and dihedral angle `cos_phi`) were strictly isolated across threads using the `private` clause, while the global energy accumulators (`e_lj` and `e_tors`) were safely aggregated using the `reduction` clause. Workload distribution was fitted to the structure of the loops: the triangular iteration space of the LJ interactions was load-balanced using `schedule(dynamic, 10)` to prevent thread starvation, whereas the uniform torsional loop utilized OpenMP's efficient default static scheduling. Finally, to maintain code portability and facilitate seamless benchmarking, the parallel regions are controlled by an `if(omp_total_energy)` conditional directive, allowing the multithreading to be toggled dynamically at run-time via external input parameters without requiring recompilation.

The `delta_energy` subroutine was optimized using OpenMP to accelerate the core $\mathcal{O}(N)$ computational bottleneck of the Monte Carlo simulation. Because evaluating a single pivot move assesses interactions between a dynamically varying number of "fixed" and "moved" atoms, the two nested iteration loops were reorganized into a perfectly nested structure and combined utilizing the `collapse(2)` clause, maximizing the total iterations available for thread distribution. To prevent substantial thread-spawning overhead from degrading simulation performance during minor structural updates, the parallel block employs a workload-aware conditional directive (`if(omp_delta_energy .and. (n_fixed * n_moved > 1000))`), ensuring parallelization is only invoked when mathematically beneficial. Thread safety was guaranteed by explicitly scoping loop control parameters and pair-specific geometric variables (`f, m, i, j, r2_old, r2_new`) as `private`, while the global interaction energy differential (`de_lj`) was accumulated securely across execution threads using a `reduction` operation.
